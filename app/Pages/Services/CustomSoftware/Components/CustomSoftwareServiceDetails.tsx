...
/** Service type */
interface CustomSoftwareService {
  title: string;
  description: string;
  icon: LucideIcon;
}

interface CustomSoftwareServiceDetailsProps {
  selectedService: CustomSoftwareService;
}

const CustomSoftwareServiceDetails: React.FC<CustomSoftwareServiceDetailsProps> = ({
  selectedService,
}) => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900/40 to-gray-800/20 rounded-3xl p-8 lg:p-12 flex flex-col justify-between backdrop-blur-sm border border-white/5 lg:h-full">
      ...
      <Link href="/contact" className="group inline-flex items-center gap-2 px-5 py-3 sm:px-8 sm:py-4  bg-amber-500 text-[#060010] rounded-xl  font-semibold text-sm sm:text-base lg:text-lg  hover:bg-amber-500 transition-all duration-300  hover:shadow-lg hover:shadow-amber-500/30 hover:scale-105">
        Get Started
...
export default CustomSoftwareServiceDetails;
