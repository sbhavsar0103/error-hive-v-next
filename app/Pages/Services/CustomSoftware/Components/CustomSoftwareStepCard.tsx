...
import CustomSoftwareListItem from "./CustomSoftwareListItem";

interface CustomSoftwareStepCardProps {
  number: number;
  title: string;
  items: string[];
  delay?: number;
...
const CustomSoftwareStepCard: React.FC<CustomSoftwareStepCardProps> = ({
  number,
  title,
  items,
  delay = 0,
      ...
      <div className="relative space-y-4">
        ...
        <ul className="space-y-3">
          {items.map((item, index) => (
            <CustomSoftwareListItem key={index} text={item} />
          ))}
        </ul>
      </div>
    </div>
...
export default CustomSoftwareStepCard;