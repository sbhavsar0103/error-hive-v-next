...
import AISolutionsStepListItem from "./AISolutionsStepListItem";

interface AISolutionsStepCardProps {
  number: number;
  title: string;
  items: string[];
  delay?: number;
...
const AISolutionsStepCard: React.FC<AISolutionsStepCardProps> = ({
  number,
  title,
  items,
  delay = 0,
      ...
      <div className="relative space-y-4">
        ...
        <ul className="space-y-3">
          {items.map((item: string, index: number) => (
            <AISolutionsStepListItem key={index} text={item} />
          ))}
        </ul>
      </div>
    </div>
...
export default AISolutionsStepCard;