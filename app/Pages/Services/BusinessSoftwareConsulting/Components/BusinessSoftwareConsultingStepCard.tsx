import BusinessSoftwareConsultingStepListItem from "./BusinessSoftwareConsultingStepListItem";

interface BusinessSoftwareConsultingStepCardProps {
  number: number;
  title: string;
  items: string[];
  delay: number;
...
export default function BusinessSoftwareConsultingStepCard({
  number,
  title,
  items,
  delay,
}: BusinessSoftwareConsultingStepCardProps) {
  return (
    <div
      ...
      <div className="relative space-y-4">
        ...
        <ul className="space-y-3">
          {items.map((item, index) => (
            <BusinessSoftwareConsultingStepListItem key={index} text={item} />
          ))}
        </ul>
      </div>
    </div>
