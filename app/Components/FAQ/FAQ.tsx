      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {items.map((item, index) => (
            <div 
              key={index} 
              className={`border border-amber-500/20 rounded-2xl bg-white/5 overflow-hidden transition-all duration-300 ${openIndex === index ? 'bg-white/10' : 'hover:bg-white/10'}`}
            >
              <button
                className="w-full text-left px-6 py-5 flex justify-between items-center focus:outline-none"
                onClick={() => toggleFAQ(index)}
              >
                <h3 className="text-lg font-semibold text-amber-500 pr-8">{item.question}</h3>
                {openIndex === index ? (
